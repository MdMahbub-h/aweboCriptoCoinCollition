	...
	initialize(function(){
		...
		game_request.request({'get_game_info': true}, function(params) {
			...
			if ('platform_data' in params) {
				temp_game_data = params['platform_data'];
			} 
...
class mainGame extends Phaser.Scene {
    ...
    preload_files() {
		let preload_sounds = ['default'];
		for (let i = 0; i < preload_sounds.length; i++) this.load.audio(preload_sounds[i], 'assets/audio/' + preload_sounds[i] + '.mp3');

        this.load.atlas("common1", "assets/common1.png", "assets/common1.json");        
		this.load.atlas(temp_user_data['current_skin'], `assets/skins/${temp_user_data['current_skin']}.png`, `assets/skins/${temp_user_data['current_skin']}.json`);
		let id = temp_user_data['current_skin'];
		let bg_key = id in temp_game_data['bgs'] ? temp_game_data['bgs'][id] : 'default';
		this.load.image(`${bg_key}_bg`, `./assets/bgs/${bg_key}.png`);
        this.load.image(`${temp_user_data['current_skin']}_logo`, './assets/logo/' + `${temp_user_data['current_skin']}.png`);
		this.load.image("main_logo", "./assets/main_logo.png");
		
		
        this.load.image("field_bg", "./assets/bg.png");

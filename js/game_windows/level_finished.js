let LevelFinished = new Phaser.Class({
    ...
    init(params) {
        ...
        let res = game_data['utils'].generate_string({
            'scene_id': 'game_windows', 'item_id': 'level_finished', 'phrase_id': '2', 'values': [], 'base_size': 35
        });
        temp = new Phaser.GameObjects.Text(this.scene, 0, -120 + add_y, res['text'], {
            ...game_data['styles']['title'], fontFamily:"font1", fontSize: res['size']
        });
        temp.setOrigin(0.5);
        this.add(temp);

...
get_random_from_2d(arr, w, h) {
	...
	load_image(key, file_url, on_complete) {		
		let loader = new Phaser.Loader.LoaderPlugin(this.scene);
		loader.image(key, file_url);
	...
	load_skin(id, on_complete) {
		let loader = new Phaser.Loader.LoaderPlugin(this.scene);
		let bg_key = id in game_data['bgs'] ? game_data['bgs'][id] : 'default';
		
		loader.image(`${id}_logo`, game_data['urls']['logo'] + `${id}.png`);
		if (!this.scene.textures.exists(`${bg_key}_bg`)) loader.image(`${bg_key}_bg`, game_data['urls']['bgs'] + `${bg_key}.png`);
		loader.atlas(id, game_data['urls']['skins'] + `${id}.png`, game_data['urls']['skins'] + `${id}.json`);
